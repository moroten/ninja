...
"build allTheThings: cat out1 out2 out3 outb1 outb2 outb3\n"
  ...
  // finish out1
  ASSERT_TRUE(plan_.EdgeFinished(edges.front(), Plan::kEdgeSucceeded, &err));
  ASSERT_TRUE(err.empty());
  edges.pop_front();
  ...
  ASSERT_TRUE(plan_.EdgeFinished(out3, Plan::kEdgeSucceeded, &err));
  ASSERT_TRUE(err.empty());

  ASSERT_FALSE(plan_.FindWork());

  for (deque<Edge*>::iterator it = edges.begin(); it != edges.end(); ++it) {
    ASSERT_TRUE(plan_.EdgeFinished(*it, Plan::kEdgeSucceeded, &err));
    ASSERT_TRUE(err.empty());
  }
  ...
  ASSERT_TRUE(plan_.EdgeFinished(last, Plan::kEdgeSucceeded, &err));
  ASSERT_TRUE(err.empty());
...
/// Verify that the deps are recorded for each output output.
