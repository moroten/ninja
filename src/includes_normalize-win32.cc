...
bool InternalGetFullPathName(LPCTSTR lpFileName, DWORD nBufferLength,
                             LPTSTR lpBuffer, LPTSTR *lpFilePart,
                             string *err) {
  DWORD result_size = GetFullPathName(lpFileName, nBufferLength,
                                      lpBuffer, lpFilePart);
  if (result_size == 0) {
    *err = string("GetFullPathName(") + lpFileName + "): " +
        GetLastErrorString();
    return false;
  } else if (result_size > nBufferLength) {
    *err = "path too long";
    return false;
  }
  return true;
...
bool SameDrive(StringPiece a, StringPiece b, string* err)  {
  ...
  if (!InternalGetFullPathName(b.AsString().c_str(),
...
string IncludesNormalize::AbsPath(StringPiece s, string* err) {
  ...
  if (!InternalGetFullPathName(s.AsString().c_str(),
      sizeof(result), result, NULL, err)) {
    return "";
  }
